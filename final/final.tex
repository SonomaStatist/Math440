...
\problem{2}{
	...
	\subproof{(c)}{
		...
		For each $n \in \bb{N}$ let $\partn_n = \cbrace{x_0,x_1,\ldots,x_{2^n}}$ partition $[0,1]$ where $x_i = \frac{i}{2^n}$. For each $n$, $\mathcal{U}(\partn_n,f) \leqslant \sum\limits_{k=1}^{2^n} 2^{1-k} \frac{1}{2^{n}} = 2^{1-n} - 2^{1-n-2^n}$ as the sup of an interval in the partition must be $f(q_1)$ or less, which means that the sup of one of the remaining intervals must be $f(q_2)$ or less, and so on up to $f(q_{2^n})$ in the final interval. Thus by Lemma 6.1.3 \[\inf\limits_{\mathcal{P}}\mathcal{U}(\partn,f) \leqslant \lim\limits_{n\to\infty} \mathcal{U}(\partn_n,f) \leqslant \lim\limits_{n\to\infty}2^{1-n} - 2^{1-n-2^n} = 0\] $f$ is non-negative, so by Theorem 6.1.4, \[0 \leqslant \sup\limits_{\mathcal{Q}}\mathcal{L}(\mathcal{Q}, f) \leqslant \inf\limits_{\mathcal{P}}\mathcal{U}(\partn,f) \leqslant 0\] Therefore $\int\limits_{0}^{1}fdx = 0$ by Definition 6.1.5.
	}
	
	\subproof{(c)}{
		$f$ is Lebesgue integrable on $[0,1]$
	}{
		$\int\limits_0^1 fdx = 0$, so $\int\limits_{0}^{1}f d\lambda = 0$ by Corollary 10.6.8.
	}
}
...
\problem{3}{
	...
	\subproof{(a)}{
		...
		Let $f,g \in \mathcal{L}(A)$ and $c \in \bb{R}$. By assumption  $\int\limits_{A}\abs{f}d\lambda$, $\int\limits_{A}\abs{g}d\lambda < \infty$, so from the Triangle Inequality, \[\abs{(f + g)(x)} = \abs{f(x) + g(x)} \leqslant \abs{f(x)} + \abs{g(x)}\] Thus $\int\limits_{A}\abs{(f + g)} d\lambda \leqslant \int\limits_{A}\abs{f} + \abs{g} d\lambda = \int\limits_{A}\abs{f} d\lambda + \int\limits_{A}\abs{g} d\lambda$ by Theorem 10.7.8(a). Therefore $\int\limits_{A}\abs{(f+g)d\lambda < \infty}$ and $(f+g) \in \mathcal{L}(A)$. Similarly, $\abs{cf(x)} = \abs{c}\cdot\abs{f(x)}$ and by Theorem 10.7.8(a), \[\int\limits_{A}\abs{cf} d\lambda = \abs{c} \cdot \int\limits_{A}\abs{f} d\lambda < \infty\] Hence $(cf)(x) \in \mathcal{L}(A)$. Because the eight vector space axioms hold trivially, $\mathcal{L}(A)$ is a vector space over $\bb{R}$.
	}
	
	\subproof{(c)}{
		Let $\norm{\cdot} : \mathcal{L}(A) \to \bb{R}$ be $\norm{f} = \int\limits_{A} \abs{f} d\lambda$. $\norm{\cdot}$ is not a norm on $\mathcal{L}(A)$
	}{
		Let $A = [0,1]$ and $f(x) = \chi_E(x)$ where $E = [0,1] \cap \bb{Q}$. Then $\norm{f} = \int\limits_{A} \abs{f} d\lambda = \int\limits_{A} \chi_E d\lambda$. By Theorem 10.6.10, $\int\limits_{A} \chi_E d\lambda = \int\limits_{E} 1 d\lambda + \int\limits_{E^c} 0 d\lambda$. Let $x_n \in E$ such that $\bigcup\limits_{n \in \bb{N}} \cbrace{x_n} = E$, and let $\epsilon > 0$ be given. Then for each $I_n = (x_n - \frac{\epsilon}{2^{n+1}}, x_n + \frac{\epsilon}{2^{n+1}})$, $E \subset \bigcup\limits_{n\in\bb{N}}I_n$ and $\sum\limits_{n \in \bb{N}} \lambda(I_n) = \epsilon$. Thus, by Theorem 6.1.11, $\lambda(E) = 0$ and $\int\limits_{E} 1 d\lambda + \int\limits_{E^c} 0 d\lambda = 1\lambda(E) + 0\lambda(E^c) = 0$. Since $f \not= 0$  for all $x \in [0,1]$, $\norm{\cdot}$ does not satisfy the property that $\norm{f} = 0$ if and only if $f = 0$, and therefore $\norm{\cdot}$ is not a norm on $\mathcal{L}(A)$.
	}
}
...
\proof{5}{
	The Hausdorff dimension of any countable subset of $\bb{R}^n$ is zero.
}{
	Let $X = \cbrace{x_1, x_2, x_3, \ldots}$ be a subset of $\bb{R}^n$, and let $C = \bigcup\limits_{k=1}^{\infty}C_k$ be a cover of $X$ such that for each $k \in \bb{N}$, $C_k = \cbrace{x_k}$. Thus $\abs{C_k} = 0$, and $C$ is a $\delta$-cover for all $\delta > 0$. By Definition D.2.4, $\mathcal{H}_\delta^s(X) \leqslant \sum\limits_{k=1}^{\infty}0^s = 0$ for all $\delta, s > 0$, and so $\mathcal{H}^s(X) \leqslant \lim\limits_{\delta \to 0^+}\mathcal{H}_\delta^s(X) = 0$ and $\dim_H(X) = 0$.
} 
